...
mod seed_tests {
    ...
    fn test_seed_verify() {
        ...
        let (new_proof, new_seed) = signer.vrf_proof_and_hash(&prev_seed.generate_next_msg(1, 1), &mut ec_vrf).unwrap();
        let seed_info = SeedInfo::from_fields(0, new_seed, new_proof);
        assert_eq!(seed_info.verify(1, 1, &prev_seed, &pub_key, &mut ec_vrf).unwrap(), true);
    }
