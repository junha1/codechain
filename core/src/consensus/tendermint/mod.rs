...
impl Tendermint {
    ...
    fn update_sealing(&self) {
        // The name should be chaned may be to "Create Proposal BLock"??
    ...
    fn submit_seal(&self, block_hash: H256, seal: Vec<Bytes>) {
        // We do not need submit_seal anymore
        // We do not have any sealings anymore
...
impl ChainNotify for TendermintChainNotify {
    ...
    fn new_blocks(
        ...
        let c = if let Some(ref weak) = *self.client.read() {
            if let Some(c) = weak.upgrade() {
                c
            } else {
                return
        ...
        let tendermint = if let Some(ref weak) = *self.tendermint.read() {
        ...
        for hash in imported {
            ...
            tendermint.to_next_height(header.number() as usize);
        }
        if !imported_is_empty {
            tendermint.to_step(Step::Commit)
        }
    }
}
